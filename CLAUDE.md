...
本文件是 **99机场梯子导航（99jc.my）** 的长期项目开发规范，供 Claude Code 在本项目中进行任何开发工作时遵循。修改本项目代码前应先阅读并遵循本文件。
...
"99机场梯子导航"是面向中文用户的正式品牌名称，"99Jichang"是英文品牌/项目名称。

所有面向用户的页面（Header、Footer、Title、Meta Description、Open Graph、JSON-LD、Breadcrumb、About、首页、文章页等）**优先使用"99机场梯子导航"**。

**禁止出现**以下未经确认的品牌写法：`99jichang`（大小写错误）、`Jichang99`（顺序颠倒）、`99机场梯子导航99Jichang`（中英重复堆叠），或其他中英混写变体。

英文品牌 `99Jichang` 只在确实需要英文品牌名、代码变量、项目名称等场景使用（如 `package.json` 的 `name` 字段、代码仓库名）。
...
面向中文用户的**机场代理与梯子工具分类导航目录**。站名本身就是"导航"二字，核心定位不是"写文章的博客"也不是"横向对比表"，而是把分散的机场品牌、推荐榜单、协议与术语百科、使用教程、故障排查整理成一份类目清晰、可以按图索骥的目录索引。
...
内容方向包括：机场代理基础知识、机场导航、机场推荐、机场评测、机场比较、性价比分析、稳定性分析、速度与延迟、节点与线路、协议知识、客户端教程、使用教程、故障排查、网络知识、行业资讯、术语解释、FAQ、长期更新型排行榜——所有这些内容类型最终都要能在首页的分类目录里找到对应入口，而不是散落在站内无人问津。
...
### 机场吧 / 机场荟 / 99机场吧 / 爱机场推荐——同一站长旗下的姊妹站，禁止内容雷同

本站是同一站长名下"机场/梯子导航评测"站群的第三个站点，姊妹站分别是：

- **机场吧**（jc8.mom）——定位"导航+知识科普"综合入口
- **机场荟**（8jc.mom）——定位"横向对比+选购决策"
- **99机场吧**（99jc.xyz）——由另一位 agent 并行建设中，本文件撰写时可能尚未上线，但规则同样适用
- **爱机场推荐**（ajc.lol）——同样并行建设中，本文件撰写时可能尚未上线，规则同样适用

这几个站点和 jichangcha.co / itizi.mom 性质不同——不是外部竞品，是自己人。但正因为如此，**更要主动避免彼此长得太像**，否则站群内部互相之间就是自我抄袭/重复内容，对所有站点的 SEO 都是减分：

- **正文文案禁止跨站复用**：哪怕是同一个 Provider 的同一批客观事实（价格、协议、线路），本站的描述、编辑结论、行文角度都必须重新组织表达，不能把其他姊妹站的句子换个域名贴过来。
- **品牌视觉必须独立**：色彩方案、Logo 图形、页面装饰风格都要和其他姊妹站区分开，不能一眼看出"这几个是同一个模板换皮"。本站的视觉身份是暖色调橙红渐变（区别于机场吧的紫蓝色系、机场荟的青绿+琥珀），Logo 是圆环+指针的罗盘/方向箭头图形（区别于机场吧的 wifi 信号弧线、机场荟的上升柱状图）。
- **网站定位需要差异化**：机场吧偏"导航+知识科普"综合入口，机场荟偏"横向对比+选购决策"；本站（99机场梯子导航，站名本身就是"导航"二字）定位是**分类目录/导航中枢**——把机场品牌、推荐榜单、协议与术语百科、使用教程、故障排查整理成清晰的类目索引，解决"从哪里开始找"的问题，首页结构是分类导航网格+排名列表，不是横向对比表，也不是排名侧栏+知识科普卡片，不能和另外两个已上线的姊妹站长得像，也要主动避开另外两个并行建设中站点可能采用的结构。
- **Provider 排行榜共享锚点规则**：综合榜**第 1 名固定为无忧链接**，这是站长对核心合作品牌的网络级统一判断，跨全部站点共享，不属于"雷同"问题；**第 2-7 名由本站独立排序**（不是照抄任一姊妹站的第 2-7 名顺序），**第 8 名及以后同样由本站独立选取**，应尽量选用其他姊妹站未收录的品牌，避免榜单名单高度重合。换句话说，"雷同"指的是文案、视觉、栏目结构层面的抄袭，不是指"共享第 1 名锚点"这件事本身。
- 各站可以互相设置友情链接（这是正常的站群操作，参考 itizi.mom / maitizi.com 页脚的友情链接做法），但内容本身必须各自独立原创。

---

## 27. 最终原则

99Jichang 不是一个"AI 批量生成文章的 SEO 网站"。
